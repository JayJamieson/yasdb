...
//
// These numeric values are PERSISTED and MUST NEVER be reused.
type KeyType byte
...
const (
	...
	// ktRefCount holds the number of forks referencing a stream. It is kept
	// out of StreamMeta, so create/delete can mutate it under metaMu without
	// racing the streamer's meta writes.
	ktRefCount KeyType = 10 // id:u64 -> count:u64

	// ktIDCounter is a singleton key holding the next StreamID to allocate.
	// It lives in its own ordinal, added by this implementation.
...
func expiryScanEnd(now uint64) []byte {
	// Deadlines are the first 8 bytes. End just past `now`, so that any
	// (now, id) entry is included.
...
// string that has p as a prefix. It is the exclusive upper bound for a
// prefix scan.
func prefixEnd(p []byte) []byte {
	...
	// p was all 0xff bytes, so there is no finite upper bound. nil means
	// "to end" for scans.
	return nil
}
